...
class NLP:
    ...
    def __setConstraints__(self):
        # self.opti.subject_to(self.states[0] == [np.pi / 3, -np.pi / 3, 0])
        self.opti.subject_to(self.dstates[0] == [0]*3)
        self.opti.subject_to(self.dstates[-1][-1] == 0.5)
        ...
        for k in range(self.N - 1):
            ...
            self.opti.subject_to(f_2['H'] @ (dq_2 - dq_1) +
                                 f_2['C'] @ (q_2 - q_1) + self.h * (- f_2['B'] @ u_2 - f_2['J_phi'].T @ lam_2) == 0)
            ...
            self.opti.subject_to(q_1 <= np.pi)
            self.opti.subject_to(q_1 >= -np.pi)

    def __setCosts__(self):
        Q = ca.diag(ca.MX([0, 0, 0]))
        R = ca.diag(ca.MX([1, 1]))
        cost = 0
